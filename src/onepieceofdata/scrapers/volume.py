"""API-based volume scraper using Fandom MediaWiki API.
...
class VolumeScraper:
    ...
    def __init__(self):
        """Initialize the API-based volume scraper."""
    ...
    def _fetch_volumes_page(self) -> BeautifulSoup:
        ...
        """
        logger.info(f"Fetching volumes page via API: {self.base_url}")

        try:
            # Use API to get rendered HTML
            html = self.api_client.get_page_html("Chapters_and_Volumes/Volumes")

            if not html:
                raise Exception("Failed to fetch volumes page HTML")
            ...
            logger.error(f"Error fetching volumes page: {str(e)}")
            raise
